		...
		if (inherits(x[[1]], 'sf')) {
			...
			if (nThreads == 1) {
				...
				if (.Platform$OS.type != 'windows') {
					...
					parallel::clusterExport(cl, c('x', 'nb', 'getEdgeIndices', 'calcPhyloMultiSite', 'spEdges', 'component'))
				}
			}	
			...
			# for (k in 1:nrow(x[[1]])) {
				...
				if (length(unique(allsites)) > 1) {
					# phylo.beta.multi(epmToPhyloComm(x, c(k, nb[[k]])), x[['phylo']])$phylo.beta.SOR
					calcPhyloMultiSite(allsites, x[['phylo']], spEdges, component)
				} else {
					0
				}
			...
			if (!slow) {
				...
				if (nThreads == 1) {
					...
					if (.Platform$OS.type != 'windows') {
						...
						cl <- parallel::makeCluster(nThreads)
						parallel::clusterExport(cl, c('x', 'datCells', 'gridCentroids', 'nb', 'getEdgeIndices', 'calcPhyloMultiSite', 'spEdges', 'component'))
					}
				}	
				...
				if (nThreads == 1) {
					...
					if (.Platform$OS.type != 'windows') {
						...
						cl <- parallel::makeCluster(nThreads)
						parallel::clusterExport(cl, c('x', 'getEdgeIndices', 'calcPhyloMultiSite', 'spEdges', 'component'))
					}
				}	
...
getEdgeIndices <- function(spEdges, taxa, tree) {
	xx <- spEdges[taxa]
	xx <- lapply(xx, function(y) setdiff(match(y, tree$edge[,2]), NA))
	unique(unlist(xx))
}
	...
	for (i in 1:length(siteList)) {
		for (j in 1:length(siteList)) {
			if (i < j) {
				...
				minDiffs <- minDiffs + min(c(pd_ij, pd_ji))
				maxDiffs <- maxDiffs + max(c(pd_ij, pd_ji))
			}
		}
	}
